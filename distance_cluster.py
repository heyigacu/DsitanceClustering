...
def index_sort(arr):
    return_arr = list(range(len(arr)))
    for i,value in enumerate(arr.argsort()[::-1]):
        return_arr[value] = i
    return return_arr
...
class CSN():
    ...
    def plot(self):
        ...
        # TODO
        # color_list = [ '#FE8D9F','#F04550','#F7E2E7','#BCE3F5','#71BBEA','cornflowerblue',
        #               'violet','turquoise','palegreen','red','hotpink', 'gold',  'mediumaquamarine',  'paleturquoise', 'powderblue',
        #                 'mediumslateblue',  'pink',  'greenyellow', 'lightseagreen'
        #             ]
        color_list = [ 'red','darkorange','gold','green','cyan','blue','purple','pink','yellowgreen','steelblue'
                    ]
        ...
        for group in groups:
            ...
            # > 1 node one group: color 
                ...
                for no in group:
                    self.node_ls[no].color = color_list[sorted_index[n]]
                    node = self.node_ls[no]
                    plt.scatter(node.coord[0],node.coord[1],s=center_point_size,color=color_list[sorted_index[n]],zorder=3)
                    if if_annotate:
                        plt.annotate(int(node.no), xy = (node.coord[0], node.coord[1]), xytext = (node.coord[0], node.coord[1]), alpha=0.5,fontsize=annotate_fontsize)
...
for i,group in enumerate(groups):
    ...
    for node in nodegroup:
        ...
        group_colors.append(node.color)
dict = {
        'No':group_sorted_ids,
        'Smiles_cluster':group_sorted_smileses,
        'Group':group_reprenstatives,
        'ClosenessCentrality':group_sorted_centrality,
        'Color':group_colors
        }
df_save = pd.DataFrame(dict)
df_save.index = group_sorted_ids
merged_df = pd.merge(df_save,df, left_index=True, right_index=True)

merged_df.to_csv(csv_save_path,sep="\t",index=None)
